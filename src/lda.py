...
class MyLDACorpus(object):
	...
	def make_dict(self, stoplist = [], V = None):
		...
		dictionary.filter_extremes(2, .7);
		return dictionary;
	def dfs(self):
...
def generateModel(project_name, filename, stopwords, K):
	...
	lda = models.ldamodel.LdaModel(corpus=reviewc, id2word = reviewc.dictionary, num_topics = K, update_every = 1, chunksize = 50, passes = 8)
