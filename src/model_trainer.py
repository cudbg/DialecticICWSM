...
class ModelTrainer(object):
	...
	def setTrainingVars(self, P, corp, num_topics, NTest, NTrain,lapp="",includeLabels=False):
		...
		with open(lapp + "exports/"+P+"/featuresAprioriLexicalPruned.txt","r") as f:
			for line in f:
	...
	def resetCurrentGen(self):
		self.currentGenerator = self.nnn*2
	...
	def genfeatures(self,json_review, liwc):
		...
		complexity = 4.71 * float(len(reviewText)) / float(len(tb.words)) + 0.5 * float(len(tb.words)) / float(len(s)) - 21.43
		flesch = None
		ARI = None
		coleman = None


		isfinit = self._assert_all_finite([complexity])

		if isfinit:
			self.addFeature(complexity, allF, "complexity measure")
		else:
			self.addFeature(0.0, allF, "complexity measure")

		try:
			flesch = textstat.flesch_kincaid_grade(reviewText)
		except:
			print("CAUGHT ERROR2")
		try:
			ARI = textstat.automated_readability_index(reviewText)
		except:
			print("CAUGHT ERROR2")
		try:
			coleman = textstat.coleman_liau_index(reviewText)
		except:
			print("CAUGHT ERROR2")

		if flesch >= 0:
			self.addFeature(flesch, allF, "flesch")
		else:
			self.addFeature(-1.0, allF, "flesch")
		if ARI >= 0:
			self.addFeature(ARI, allF, "ARI")
		else:
			self.addFeature(-1.0, allF, "ARI")
		if coleman >= 0:
			self.addFeature(coleman, allF, "coleman")
		else:
			self.addFeature(-1.0, allF, "coleman")
...
#
		...
		#		keep.append(0)
#
		#isOutputFinite = self._assert_all_finite(allF)
		#print(isOutputFinite)
		#print(isOutputFinite)
		#print(isOutputFinite)


		lexicalDiversity = ((float(len(set(tb.words))) / float(len(tb.words))) * 100)
		self.addFeature(lexicalDiversity, allF, "lexical diversity")
		#self.addFeature(lexicalDiversity * log(1+len(s),10), allF, "lexical diversity, discounting for length")
		...
		flesch_ease = None
		try:
			flesch_ease = textstat.flesch_reading_ease(reviewText)
		except:
			print("CAUGHT ERROR-flesch")

		if flesch_ease >= 0:
			self.addFeature(flesch_ease, allF, "flesch reading ease")
		else:
			self.addFeature(-1.0, allF, "flesch reading ease")


		fog = None
		try:
			fog = textstat.gunning_fog(reviewText)
		except:
			print("CAUGHT ERROR-fog")

		if fog >= 0:
			self.addFeature(fog, allF, "FOG")
		else:
			self.addFeature(-1.0, allF, "FOG")


		# SMOG = None
		# try:
		# 	SMOG = textstat.gunning_fog(reviewText)
		# except:
		# 	print("CAUGHT ERROR-fog")
		...
		self.addFeature(sumLengths/float(len(tb.words)), allF, "average word length")


		liwc_vars = [json_review["LIWC"]["contents"][0]["liwc_scores"]["categories"]["friend"],json_review["LIWC"]["contents"][0]["liwc_scores"]["categories"]["social"],json_review["LIWC"]["contents"][0]["liwc_scores"]["categories"]["posemo"],json_review["LIWC"]["contents"][0]["liwc_scores"]["categories"]["ipron"],json_review["LIWC"]["contents"][0]["liwc_scores"]["categories"]["informal"],json_review["LIWC"]["contents"][0]["liwc_scores"]["categories"]["affect"],json_review["LIWC"]["contents"][0]["liwc_scores"]["categories"]["we"],json_review["LIWC"]["contents"][0]["liwc_scores"]["authentic"],json_review["LIWC"]["contents"][0]["receptiviti_scores"]["percentiles"]["social_skills"],json_review["LIWC"]["contents"][0]["receptiviti_scores"]["percentiles"]["openness"],json_review["LIWC"]["contents"][0]["receptiviti_scores"]["percentiles"]["cooperative"],json_review["LIWC"]["contents"][0]["receptiviti_scores"]["percentiles"]["friend_focus"],json_review["LIWC"]["contents"][0]["receptiviti_scores"]["percentiles"]["friendly"],json_review["LIWC"]["contents"][0]["receptiviti_scores"]["percentiles"]["sociable"]]
		if (liwc):
		 	for q in range(0,len(liwc_vars)):
				self.addFeature(liwc_vars[q], allF, "LIWC var # " + str(q))


		liwc_vars_grammar = [json_review["LIWC"]["contents"][0]["liwc_scores"]["categories"]["Comma"],json_review["LIWC"]["contents"][0]["liwc_scores"]["categories"]["article"],json_review["LIWC"]["contents"][0]["liwc_scores"]["categories"]["conj"],json_review["LIWC"]["contents"][0]["liwc_scores"]["categories"]["Parenth"],json_review["LIWC"]["contents"][0]["liwc_scores"]["categories"]["article"],json_review["LIWC"]["contents"][0]["liwc_scores"]["categories"]["auxverb"],json_review["LIWC"]["contents"][0]["liwc_scores"]["categories"]["focuspast"],json_review["LIWC"]["contents"][0]["liwc_scores"]["categories"]["Exclam"],json_review["LIWC"]["contents"][0]["liwc_scores"]["categories"]["focuspresent"],json_review["LIWC"]["contents"][0]["liwc_scores"]["categories"]["adverb"],json_review["LIWC"]["contents"][0]["liwc_scores"]["categories"]["quant"],json_review["LIWC"]["contents"][0]["liwc_scores"]["categories"]["prep"],json_review["LIWC"]["contents"][0]["liwc_scores"]["categories"]["Dash"],json_review["LIWC"]["contents"][0]["liwc_scores"]["categories"]["negate"]]
		if (liwc):
			for q in range(0,len(liwc_vars_grammar)):
				self.addFeature(liwc_vars_grammar[q], allF, "GRAMMARLWIC# " + str(q))
	...
	def trainClassifier(self,P, data_file, cutoff, classifierDump, vectorizerDump, explanationDump, liwc, deploy=False):
		...
		if not deploy:
			...
			if self.generateFeatures:
				...
				for r in data:
					...
					#print(r)
					i+=1
					fff = self.genfeatures(r, liwc)
					X.append(fff)

				for i in range(0,self.T):
					print(i)
					spam_test.append(self.genfeatures(unhelpful_exp[i],liwc))
				for i in range(0,self.T):
					print(i)
					if i == self.T-1:
						self.lastOne = True
					ham_test.append(self.genfeatures(helpful_exp[i],liwc))




			print("TESTING FINITIY")
			print(self._assert_all_finite(X))
			for i in X:
				if not self._assert_all_finite(X):
					print(False)
